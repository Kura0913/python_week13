...
class AddStuWidget(QtWidgets.QWidget):
    ...
    def click_query(self):
        if self.name_editor.text() == "":
            self.setMsg("Name editor can't be empty", "red", True)
        else:
    ...
    def action_query(self, result):
        ...
        if result["status"] == "Fail":
            ...
            self.setMsg(f"Query Success, Please enter subjects for student {self.name_editor.text()}", "green")
        else:
            self.setMsg("Query Fail, Name is already taken", "red", True)

    def click_add(self):
        if self.subject_editor.text() == "" or self.score_editor.text() == "" or self.subject_editor.text() == "Subject":
            self.setMsg("Subject or Score editor can't be empty, Please refill it", "red")
        else:
            if self.subject_editor.text() not in self.add_scorelist:
                self.add_scorelist[self.subject_editor.text()] = self.score_editor.text()
                self.setMsg(f"Student {self.name_editor.text()}'s subject '{self.subject_editor.text()}' with score '{self.score_editor.text()}' added", "green")
                self.Sendbutton.setEnabled(True)
            else:
                self.setMsg(f"The {self.name_editor.text()}'s subject '{self.subject_editor.text()}' is already exist", "red")
    ...
    def setMsg(self, text, color, setReset=False):
